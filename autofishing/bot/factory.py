...
from autofishing.config import bot_settings_from_dict, resolve_adb_path
...
def build_bot_from_config(cfg: dict) -> FishingBot:
    model_cfg = cfg["model"]
    backend = cfg.get("backend", "screen")
    settings = bot_settings_from_dict(cfg)
    ...
    if backend in ("window", "bluestacks", "bs"):
        ...
        clicker = BlueStacksClicker(image_width=img_w, image_height=img_h)
        print(f"[bot] backend=window logical={img_w}x{img_h}")
        return FishingBot(capture, detector, settings, clicker=clicker)
    ...
    if backend == "adb":
        ...
        if not region.width or not region.height:
            region = AdbRegion()
            print("[adb] detect ROI: full screen")
        else:
            print(
                f"[adb] detect ROI: left={region.left} top={region.top} "
        ...
        print(f"[adb] connected {device.serial} wm={w}x{h}")
        clicker = BlueStacksClicker(image_width=img_w, image_height=img_h)
        return FishingBot(device, detector, settings, clicker=clicker)
    ...
    return FishingBot(capture, detector, settings, clicker=ScreenClicker())
